...
if __name__ == "__main__":
    ...
    with KCEXTool("auth.json", headless=False) as k:
        snaps = k.get_multi_snapshot(["PAXG", "XAUT"])
        p = snaps["PAXG"].price
        x = snaps["XAUT"].price
        print(f"PAXG={p} XAUT={x} spread={p-x} src={snaps['PAXG'].source}/{snaps['XAUT'].source}")
        res = k.two_legs_trade("long_paxg", action="close", order_type="market", amount_usdt="30")
        print(res)